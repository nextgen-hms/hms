...
\restrict f7o030IyHLcXU9ZYof7f9VT2UmOIcX9mhponbj5kL7MbC5WftW5ylQAIu9T8YcX
...
begin
  if TG_OP = 'INSERT' then
    insert into medicine_transaction(medicine_id, txn_type, qty, amount_per_unit, ref_purchase_id)
    values (NEW.medicine_id, 'purchase', NEW.qty, NEW.unit_cost, NEW.purchase_id);
...
begin
  if TG_OP = 'INSERT' then
    insert into medicine_transaction(medicine_id, txn_type, qty, amount_per_unit, ref_purchase_return_id)
    values (NEW.medicine_id, 'purchase_return', NEW.qty, NEW.unit_cost, NEW.purchase_return_id);
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    INSERT INTO medicine_transaction(medicine_id, txn_type, qty, amount_per_unit, ref_sale_id)
    VALUES (NEW.medicine_id, 'sale', NEW.qty, NEW.unit_price, NEW.sale_id);
...
BEGIN
    INSERT INTO medicine_transaction(
        ...
        qty,
        amount_per_unit,
        ref_sale_return  -- ✅ correct column
    )
    VALUES (
        NEW.medicine_id,
        'sale_return',
        NEW.qty,
        NEW.unit_price,
        NEW.return_id    -- ✅ references return_id from sale_return_detail
    );
...
BEGIN
    ...
    -- Calculate quantity adjustment (considering sub-quantities as decimal)
    v_qty_adjustment := new.qty;

    IF TG_OP = 'INSERT' THEN
        UPDATE medicine 
        SET stock_quantity = stock_quantity + (v_sign * v_qty_adjustment)
        WHERE medicine_id = new.medicine_id;

    ELSIF TG_OP = 'DELETE' THEN
        UPDATE medicine
        SET stock_quantity = stock_quantity - (v_sign * old.qty)
 ...
 SELECT medicine_id,
    ...
    is_active,
        ...
        CASE
            ...
            WHEN (expiry_date <= (CURRENT_DATE + '90 days'::interval)) THEN 'Near Expiry'::text
            ELSE 'Valid'::text
...
\unrestrict f7o030IyHLcXU9ZYof7f9VT2UmOIcX9mhponbj5kL7MbC5WftW5ylQAIu9T8YcX
