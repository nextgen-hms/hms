...
describe("pharmacy transaction detail routes", () => {
  ...
  it("updates existing sale details using the live pharmacy_sale_detail_id key", async () => {
    queryMock
      ...
      .mockResolvedValueOnce({})
      .mockResolvedValueOnce({
        rows: [{ pharmacy_sale_detail_id: 17, medicine_id: 5, batch_id: 5 }],
      })
      .mockResolvedValueOnce({})
      .mockResolvedValueOnce({})
      .mockResolvedValueOnce({});
    ...
    expect(response.status).toBe(200);
    expect(queryMock.mock.calls[2]?.[0]).toContain("pharmacy_sale_detail_id");
    expect(queryMock.mock.calls[3]?.[0]).toContain("WHERE pharmacy_sale_detail_id = $8");
    expect(releaseMock).toHaveBeenCalledTimes(1);
  });
